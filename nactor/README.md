...
| method | path | does |
| --- | --- | --- |
| GET | `/api/health` | liveness; identity names + `directorsConfigured` count; **no secrets, no auth** |
| GET | `/api/state` | `directors`, `nactorAddress`, identities (npub/signer/status), channels, tiers, queue, history |
